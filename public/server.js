...
async function updateGraph(process) {
	...
	response.cpu.forEach((item, index)=>{response.cpu[index].x = new Date(item.x); response.cpu[index].y = parseFloat(item.y >= 1 ? item.y : 0);})
	response.ram.forEach((item, index)=>{response.ram[index].x = new Date(item.x); response.ram[index].y = parseFloat(item.y >= 1 ? item.y : 0);})
	response.up.forEach((item, index)=>{response.up[index].x = new Date(item.x); response.up[index].y = parseFloat(item.y >= 1 ? item.y : 0);})
	response.down.forEach((item, index)=>{response.down[index].x = new Date(item.x); response.down[index].y = parseFloat(item.y >= 1 ? item.y : 0);})
